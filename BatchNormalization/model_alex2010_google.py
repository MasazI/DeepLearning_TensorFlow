...
def inference(images):
    ...
    with tf.variable_scope('conv1') as scope:
        ...
        eps=1e-5
        gamma = weight_variable([64])
        beta = weight_variable([64])
        mean, variance = tf.nn.moments(images, [0])
        bn1 = tf.nn.batch_normalization(conv, mean=mean, variance=variance, offset=beta, scale=gamma, variance_epsilon=eps)
        #bn1 = batch_normalization(64, conv)
    ...
    with tf.variable_scope('conv2') as scope:
        ...
        #bias = tf.nn.bias_add(conv, biases)
        bn2 = batch_normalization(64, conv)
    ...
    with tf.variable_scope('local3') as scope:
        ...
        bn3 = batch_normalization(384, tf.matmul(reshape, weights))
        local3 = tf.nn.relu(bn3)
    ...
    with tf.variable_scope('local4') as scope:
        ...
        bn4 = batch_normalization(192, tf.matmul(local3, weights))
        local4 = tf.nn.relu(bn4)
